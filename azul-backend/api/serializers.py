from rest_framework import serializers
from .models import Author, Saga, Book, Character, Place, Thing, Word


# ── Element serializers ────────────────────────────────────────────────────────

class CharacterSerializer(serializers.ModelSerializer):
    class Meta:
        model = Character
        fields = ['id', 'name', 'age', 'description']


class PlaceSerializer(serializers.ModelSerializer):
    class Meta:
        model = Place
        fields = ['id', 'name', 'description']


class ThingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Thing
        fields = ['id', 'name', 'description']


class WordSerializer(serializers.ModelSerializer):
    class Meta:
        model = Word
        fields = ['id', 'word', 'description']


# ── Library view ───────────────────────────────────────────────────────────────

class AuthorListSerializer(serializers.ModelSerializer):
    """Minimal author info for the library list (saga_count, book_count annotated)."""
    saga_count = serializers.IntegerField(read_only=True)
    book_count = serializers.IntegerField(read_only=True)

    class Meta:
        model = Author
        fields = ['id', 'name', 'description', 'saga_count', 'book_count']


# ── Author detail view ─────────────────────────────────────────────────────────

class BookInSagaSerializer(serializers.ModelSerializer):
    """Compact book used inside a saga row in the author view (no description needed)."""
    saga_id = serializers.UUIDField(source='saga_id', read_only=True, allow_null=True)

    class Meta:
        model = Book
        fields = ['id', 'title', 'start_date', 'end_date', 'saga_id']


class SagaInAuthorSerializer(serializers.ModelSerializer):
    """Saga with its books for the author view."""
    books = BookInSagaSerializer(source='saga_books', many=True, read_only=True)

    class Meta:
        model = Saga
        fields = ['id', 'name', 'description', 'books']


class BookOverviewSerializer(serializers.ModelSerializer):
    """Book with element count — used for both standalone books and saga books in overview."""
    element_count = serializers.SerializerMethodField()
    saga_id = serializers.UUIDField(source='saga_id', read_only=True, allow_null=True)

    class Meta:
        model = Book
        fields = ['id', 'title', 'description', 'start_date', 'end_date', 'saga_id', 'element_count']

    def get_element_count(self, obj):
        return (
            getattr(obj, 'char_count', 0) +
            getattr(obj, 'place_count', 0) +
            getattr(obj, 'thing_count', 0) +
            getattr(obj, 'word_count', 0)
        )


class AuthorDetailSerializer(serializers.ModelSerializer):
    """Full author detail: sagas (each with compact books) + standalone books with element counts."""
    sagas = SagaInAuthorSerializer(many=True, read_only=True)
    standalone_books = BookOverviewSerializer(many=True, read_only=True)

    class Meta:
        model = Author
        fields = ['id', 'name', 'description', 'sagas', 'standalone_books']


# ── Saga detail view ───────────────────────────────────────────────────────────

class SagaDetailSerializer(serializers.ModelSerializer):
    author_id = serializers.UUIDField(source='author.id', read_only=True)
    author_name = serializers.CharField(source='author.name', read_only=True)
    books = BookOverviewSerializer(source='saga_books', many=True, read_only=True)

    class Meta:
        model = Saga
        fields = ['id', 'name', 'description', 'author_id', 'author_name', 'books']


# ── Book detail view ───────────────────────────────────────────────────────────

class BookDetailSerializer(serializers.ModelSerializer):
    author_id = serializers.UUIDField(source='author.id', read_only=True)
    saga_id = serializers.UUIDField(source='saga_id', read_only=True, allow_null=True)
    characters = CharacterSerializer(many=True, read_only=True)
    places = PlaceSerializer(many=True, read_only=True)
    things = ThingSerializer(many=True, read_only=True)
    words = WordSerializer(many=True, read_only=True)

    class Meta:
        model = Book
        fields = [
            'id', 'title', 'description', 'start_date', 'end_date',
            'author_id', 'saga_id',
            'characters', 'places', 'things', 'words',
        ]


# ── Mention entities (autocomplete) ───────────────────────────────────────────
# Returns only names/ids — no descriptions — to keep the payload tiny.

class MentionWordSerializer(serializers.ModelSerializer):
    class Meta:
        model = Word
        fields = ['id', 'word']


class MentionCharacterSerializer(serializers.ModelSerializer):
    class Meta:
        model = Character
        fields = ['id', 'name']


class MentionPlaceSerializer(serializers.ModelSerializer):
    class Meta:
        model = Place
        fields = ['id', 'name']


class MentionThingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Thing
        fields = ['id', 'name']


class MentionBookSerializer(serializers.ModelSerializer):
    characters = MentionCharacterSerializer(many=True, read_only=True)
    places = MentionPlaceSerializer(many=True, read_only=True)
    things = MentionThingSerializer(many=True, read_only=True)
    words = MentionWordSerializer(many=True, read_only=True)

    class Meta:
        model = Book
        fields = ['id', 'title', 'characters', 'places', 'things', 'words']


class MentionSagaSerializer(serializers.ModelSerializer):
    class Meta:
        model = Saga
        fields = ['id', 'name']


class MentionAuthorSerializer(serializers.ModelSerializer):
    sagas = MentionSagaSerializer(many=True, read_only=True)
    books = MentionBookSerializer(many=True, read_only=True)

    class Meta:
        model = Author
        fields = ['id', 'name', 'sagas', 'books']


# ── Write serializers (input validation) ──────────────────────────────────────

class AuthorWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Author
        fields = ['name', 'description']


class SagaWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Saga
        fields = ['name', 'description']


class BookWriteSerializer(serializers.ModelSerializer):
    saga_id = serializers.UUIDField(allow_null=True, required=False)

    class Meta:
        model = Book
        fields = ['title', 'description', 'start_date', 'end_date', 'saga_id']

    def validate_saga_id(self, value):
        if value is None:
            return None
        try:
            Saga.objects.get(pk=value)
        except Saga.DoesNotExist:
            raise serializers.ValidationError('Saga not found.')
        return value


class CharacterWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Character
        fields = ['name', 'age', 'description']


class PlaceWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Place
        fields = ['name', 'description']


class ThingWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Thing
        fields = ['name', 'description']


class WordWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Word
        fields = ['word', 'description']
