...
export default function SagaView({ sagaId, navigate }) {
  ...
  function openAddBook() { setForm({ title: "", author: saga.author || "", description: "", startDate: "", endDate: "" }); setFormError(""); setModal("add-book") }
  function openEditBook(book) { setForm({ title: book.title, author: book.author || "", description: book.description, startDate: book.startDate || "", endDate: book.endDate || "" }); setFormError(""); setModal({ type: "edit-book", book }) }
  ...
  async function submitBook(e) {
    ...
    if (modal === "add-book") {
      await dispatch({ type: "ADD_BOOK", sagaId, title: trimmed, author: form.author.trim(), description: form.description.trim(), startDate: form.startDate || null, endDate: form.endDate || null })
    } else {
  ...
  async function submitAppend(e) {
    ...
    await dispatch({ type: "APPEND_TO_DESCRIPTION", target: "saga", sagaId, text: appendText.trim() })
    setAppendTx(""); setAppend(false); refresh()
  }

  const mentionNav = entity => {
    if (entity.type === "saga") navigate({ view: "saga", sagaId: entity.id })
    else if (entity.type === "book") navigate({ view: "book", bookId: entity.id, sagaId: entity.sagaId || null })
    else if (entity.bookId) navigate({ view: "book", bookId: entity.bookId, sagaId: null, tab: entity.type + "s" })
  }
    ...
    <div className="fade-in space-y-5">
      <div className="card p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="flex gap-1 shrink-0">
            <IconBtn onClick={() => { setForm({ name: saga.name, author: saga.author || "", description: saga.description }); setModal("edit-saga") }}><EditIcon /></IconBtn>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={openAddBook} className="btn-primary flex items-center gap-1.5 py-2 px-3 text-xs"><PlusIcon /> New book</button>
        </div>
        <div className="space-y-3">
          {books.length === 0
            ...
            : books.map(book => {
                ...
                return (
                  <div key={book.id} className="card p-4 cursor-pointer" onClick={() => navigate({ view: "book", bookId: book.id, sagaId })}>
      ...
      <Modal isOpen={modal === "edit-saga"} onClose={() => setModal(null)}>
        <form onSubmit={async e => { e.preventDefault(); await dispatch({ type: "UPDATE_SAGA", sagaId, updates: { name: form.name, author: form.author, description: form.description } }); setModal(null); refresh() }} className="space-y-4">
