...
export default function AuthorView({ authorId, navigate }) {
  ...
  useEffect(() => {
    setAuthor(null)
    api.getAuthor(authorId).then(setAuthor).catch(console.error)
  }, [authorId, revision])

  if (!author) return <div className="text-cream/60 font-body text-center py-12">Loading…</div>

  function refresh() { setRevision(r => r + 1) }

  // ── Sagas ──────────────────────────────────────────────────────────────────
  function openAddSaga() { setForm({ name: '', description: '' }); setFormError(''); setModal('add-saga') }
  function openEditSaga(saga) { setForm({ name: saga.name, description: saga.description }); setFormError(''); setModal({ type: 'edit-saga', saga }) }

  async function submitSaga(e) {
    e.preventDefault()
    if (!form.name.trim()) return
    const trimmed = form.name.trim()
    const duplicate = author.sagas.some(
      s => s.name.toLowerCase() === trimmed.toLowerCase() &&
           (modal === 'add-saga' || s.id !== modal.saga.id)
    )
    if (duplicate) { setFormError('A saga with that name already exists'); return }
    if (modal === 'add-saga') {
      await dispatch({ type: 'ADD_SAGA', authorId, name: trimmed, description: form.description.trim() })
    } else {
      await dispatch({ type: 'UPDATE_SAGA', authorId, sagaId: modal.saga.id, updates: { name: trimmed, description: form.description.trim() } })
    }
    setModal(null); refresh()
  }

  // ── Books ──────────────────────────────────────────────────────────────────
  function openAddBook(sagaId = null) { setForm({ title: '', description: '', startDate: '', endDate: '', sagaId }); setFormError(''); setModal('add-book') }
  function openEditBook(book) { setForm({ title: book.title, description: book.description, startDate: book.startDate || '', endDate: book.endDate || '', sagaId: book.sagaId }); setFormError(''); setModal({ type: 'edit-book', book }) }

  async function submitBook(e) {
    e.preventDefault()
    if (!form.title.trim()) return
    const trimmed = form.title.trim()
    // Collect all books from all sagas + standaloneBooks for duplicate check
    const allBooks = [...(author.standaloneBooks || []), ...(author.sagas || []).flatMap(s => s.books || [])]
    ...
    if (modal === 'add-book') {
      await dispatch({ type: 'ADD_BOOK', authorId, sagaId: form.sagaId || null, title: trimmed, description: form.description.trim(), startDate: form.startDate || null, endDate: form.endDate || null })
    } else {
  ...
  function deleteSaga(sagaId) {
    setConfirmDialog({ message: 'Delete this saga? Books will become standalone.', onConfirm: async () => { await dispatch({ type: 'DELETE_SAGA', authorId, sagaId }); refresh() } })
  }

  async function submitAppend(e) {
    e.preventDefault()
    if (!appendText.trim()) return
    await dispatch({ type: 'APPEND_TO_DESCRIPTION', target: 'author', authorId, text: appendText.trim() })
    setAppendText(''); setAppendModal(false); refresh()
  }
  ...
  return (
    <div className="fade-in space-y-5">
      ...
      <div className="card p-5">
        ...
        {author.description
          ? <DescriptionText text={author.description} state={state} onMentionClick={mentionNav} />
      ...
      <Section title="Sagas" onAdd={openAddSaga} addLabel="New saga">
        {(!author.sagas || author.sagas.length === 0)
          ? <EmptyCard Icon={BookOpenIcon} text="No sagas yet" />
          : author.sagas.map(saga => {
              const sagaBooks = saga.books || []
              return (
                <div key={saga.id} className="card overflow-hidden">
                  <div className="p-4 cursor-pointer hover:bg-teal-pale/30 transition-colors" onClick={() => navigate({ view: 'saga', sagaId: saga.id, authorId })}>
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-xs font-sans text-ink-muted mt-1 inline-block">
                          {sagaBooks.length} {sagaBooks.length === 1 ? 'book' : 'books'}
                        </span>
                      </div>
                      <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                        <IconBtn onClick={() => openAddBook(saga.id)} title="Add book to saga"><PlusIcon /></IconBtn>
                        <IconBtn onClick={() => openEditSaga(saga)} title="Edit saga"><EditIcon /></IconBtn>
                        <IconBtn onClick={() => deleteSaga(saga.id)} title="Delete saga" danger><TrashIcon /></IconBtn>
                      </div>
                    </div>
                  </div>
                  {sagaBooks.length > 0 && (
                    <div className="border-t border-cream-border">
                      {sagaBooks.map(book => (
                        <BookRow key={book.id} book={book}
                          onClick={() => navigate({ view: 'book', bookId: book.id, authorId, sagaId: saga.id })}
      ...
      <Section title="Standalone books" onAdd={() => openAddBook(null)} addLabel="New book">
        {(!author.standaloneBooks || author.standaloneBooks.length === 0)
          ? <EmptyCard Icon={BookmarkIcon} text="No standalone books" />
          : author.standaloneBooks.map(book => (
              <BookCard key={book.id} book={book}
                onClick={() => navigate({ view: 'book', bookId: book.id, authorId, sagaId: null })}
                onEdit={() => openEditBook(book)}
                onDelete={() => deleteBook(book.id)}
              />
            ))
      ...
      <Modal isOpen={modal === 'edit-author'} onClose={() => setModal(null)} title="Edit author">
        <form onSubmit={async e => { e.preventDefault(); await dispatch({ type: 'UPDATE_AUTHOR', id: authorId, updates: { name: form.name, description: form.description } }); setModal(null); refresh() }} className="space-y-4">
          ...
          <FormActions onCancel={() => setModal(null)} submitLabel="Save" />
        </form>
      </Modal>
      ...
      <Modal isOpen={modal === 'add-book' || modal?.type === 'edit-book'} onClose={() => setModal(null)} title={modal === 'add-book' ? 'New book' : 'Edit book'}>
        <form onSubmit={submitBook} className="space-y-4">
          ...
          {modal === 'add-book' && author.sagas?.length > 0 && (
            <div>
              ...
              <select className="input" value={form.sagaId || ''} onChange={e => setForm(f => ({ ...f, sagaId: e.target.value || null }))}>
                ...
                {author.sagas.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
          )}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onAdd} className="btn-primary flex items-center gap-1.5 py-2 px-3 text-xs">
          <PlusIcon />{addLabel}
        </button>
      </div>
